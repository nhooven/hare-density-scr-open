...
# SCRIPT: 07b - Full open model (modified linear predictors)
# AUTHOR: Nate Hooven
...
model.code <- nimbleCode({
  ...
  rho_b0 ~ dunif(log(0.1), log(7))
  rho_sd ~ dexp(1)

  # phi - persistence (logit scale)
  phi_b0 ~ dnorm(0, sd = 1.5)
  phi_sd ~ dexp(1)
    
  # linear predictor coefficients (n = 6)
  for (b in 1:6) {
      
    rho_b[b] ~ dnorm(0, sd = 1)
    phi_b[b] ~ dnorm(0, sd = 1)
  ...
  for (u in 1:U) {
    ...
    for (t in (first.year[u] + 1):YR) {
      ...
      # rho
      log(rho[u, t]) <- rho_b0 + 
                        rho_sd * rho_c[site.clust[u]] +
                        rho_b[1] * site.ret[u] +
                        rho_b[2] * site.ret[u] * post1[t] +
                        rho_b[3] * site.ret[u] * post2[t] +
                        rho_b[4] * site.pil[u] +
                        rho_b[5] * site.pil[u] * post1[t] +
                        rho_b[6] * site.pil[u] * post2[t]
  ...
  for (u in 1:U) {
    ...
    for (t in (first.year[u]):(YR - 1)) {
      ...
      # phi
      logit(phi[u, t]) <- phi_b0 + 
                          phi_sd * phi_c[site.clust[u]] +
                          phi_b[1] * site.ret[u] +
                          phi_b[2] * site.ret[u] * post1[t] +
                          phi_b[3] * site.ret[u] * post2[t] +
                          phi_b[4] * site.pil[u] +
                          phi_b[5] * site.pil[u] * post1[t] +
                          phi_b[6] * site.pil[u] * post2[t]
      
      # ENTERED
      omega[2, 1, u, t] <- 0.0
      omega[2, 2, u, t] <- phi[u, t]
      omega[2, 3, u, t] <- 1 - phi[u, t]
  ...
  alpha0_sd ~ dexp(1)           # SD
  alpha0_b1 ~ dnorm(0, sd = 1)  
  alpha0_b2 ~ dnorm(0, sd = 1)  
  alpha0_b3 ~ dnorm(0, sd = 1)  
  
  # alpha2 - trap response
  alpha2_b0 ~ dnorm(0, sd = 1)  # intercept
  alpha2_sd ~ dexp(1)           # SD
  alpha2_b1 ~ dnorm(0, sd = 1)
  alpha2_b2 ~ dnorm(0, sd = 1)
  alpha2_b3 ~ dnorm(0, sd = 1)
  
  # sigma - spatial scale of movement
  sigma_b0 ~ dnorm(log(45), sd = 0.5)     # intercept
  sigma_sd ~ dexp(1)                      # SD
  sigma_b1 ~ dnorm(0, sd = 1)             
  sigma_b2 ~ dnorm(0, sd = 1)
  sigma_b3 ~ dnorm(0, sd = 1)
  
  # cluster-specific random scaling factors
  for (c in 1:4) {
    
    alpha0_c[c] ~ dnorm(0, sd = 1)
  ...
  for (i in 1:M) {
    ...
    alpha2[i] <- alpha2_b0 +                          # mean intercept
                 alpha2_sd * alpha2_c[cluster[i]] +   # cluster random intercept
                 alpha2_b1 * sex[i] +                 # male effect
                 alpha2_b2 * ret[i] +                 # retention effect
                 alpha2_b3 * pil[i]                   # piling effect
    
    # sigma - spatial scale of movement
    log(sigma[i]) <- sigma_b0 +                       # mean intercept
                     sigma_sd * sigma_c[cluster[i]] + # cluster random intercept
                     sigma_b1 * sex[i] +              # male effect
                     sigma_b2 * ret[i] +              # retention effect
                     sigma_b3 * pil[i]                # piling effect
    ...
    for (t in (first.year[site[i]]):YR) {
      ...
      for (k in 1:K[site[i], t]) {
        ...
        log(alpha0[i, k, t]) <- alpha0_b0 +                          # mean intercept
                                alpha0_sd * alpha0_c[cluster[i]] +   # cluster random intercept
                                alpha0_b1 * sex[i] +                 # effect of male
                                alpha0_b2 * ret[i] +                 # retention effect
                                alpha0_b3 * pil[i] +                 # piling effect
                                alpha2[i] * prev.cap[i, k, t]        # previous cap effect
...
inits <- list(
  ...
  # CLOSED
  s = array(c(cbind(runif(constant.list$M, -200, 200),       # these are wrong
  ...
  alpha0_b2 = rnorm(1, 0, 1),
  alpha0_b3 = rnorm(1, 0, 1),
  alpha0_c = rnorm(4, 0, 1),
